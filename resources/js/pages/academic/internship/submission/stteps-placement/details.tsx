...
import { Save, AlertCircle, CheckCircle2, Clock, Building2, User, Calendar, Briefcase, Info, ChevronDown, Search, Plus, X, Loader2 } from "lucide-react";
...
import { stepDetailsSchema } from "./detail.schema";
...
export function Details({ data, setData, errors = {}, currentPlacement, form: passedForm }: { data: any, setData: any, errors?: any, currentPlacement?: any, form?: any }) {
    const [loading, setLoading] = useState(false);
    const [verifying, setVerifying] = useState(false);
    const [companyFound, setCompanyFound] = useState<boolean | null>(currentPlacement?.company?.id ? true : null);
    ...
    useEffect(() => {
        if (currentPlacement?.company?.ruc) {
            fetch(`/api/internship/companies/verify/${currentPlacement.company.ruc}`)
                .then(r => r.json())
    ...
    // Sincronizar cambios locales con el estado de Inertia en el padre
    const { watch } = form;
    useEffect(() => {
        const subscription = watch((value: any) => {
            setData((prev: any) => ({
                ...prev,
                company: { ...prev.company, ...value.company },
                placement: { ...prev.placement, ...value.placement }
            }));
        });
    ...
    const handleUpdate = () => {
        ...
        router.patch(internship.placements.update.url(currentPlacement.id), {
            company: data.company,
            placement: data.placement
        }, {
            onBefore: () => setLoading(true),
            onFinish: () => setLoading(false),
            onSuccess: () => { /* toast gestionado por ExceptionHandler */ },
            onError: () => { /* errores de validación: ExceptionHandler no los cubre */ }
        });
    };

    const labelStyle = "text-[10px] uppercase font-bold text-muted-foreground tracking-widest ml-1 mb-1";
    const isCompanyLocked = !!currentPlacement;

    return (
        <TooltipProvider delayDuration={200}>
            <Form {...form}>
                <div className="animate-in fade-in zoom-in-95 duration-500">
                    <section id="step-2" className="max-w-4xl mx-auto p-4 md:p-8 space-y-12">

                        {/* Alertas de Estado */}
                        {currentPlacement && (
                            <div className="space-y-4">
                                {currentPlacement.approval_status === 3 && (
                                    <Alert variant="destructive" className="bg-red-50/50 border-red-200">
                                        <AlertCircle className="h-4 w-4 text-red-600" />
                                        <AlertTitle className="text-red-900 font-bold uppercase text-xs tracking-tight">Información Observada</AlertTitle>
                                        <AlertDescription className="text-red-800 text-xs mt-1 leading-relaxed">
                                            {currentPlacement.observation || "Por favor revise los datos de la empresa o el jefe directo."}
                                        </AlertDescription>
                                    </Alert>
                                )}
                                {currentPlacement.approval_status === 1 && (
                                    <Alert className="bg-green-50/50 border-green-200">
                                        <CheckCircle2 className="h-4 w-4 text-green-600" />
                                        <AlertTitle className="text-green-900 font-bold uppercase text-xs tracking-tight">Datos Aprobados</AlertTitle>
                                        <AlertDescription className="text-green-800 text-xs mt-1 leading-relaxed">
                                            La información de la empresa y jefe ha sido validada exitosamente.
                                        </AlertDescription>
                                    </Alert>
                                )}
                                {currentPlacement.approval_status === 2 && (
                                    <Alert className="bg-amber-50/50 border-amber-200">
                                        <Clock className="h-4 w-4 text-amber-600" />
                                        <AlertTitle className="text-amber-900 font-bold uppercase text-xs tracking-tight">Datos en Revisión</AlertTitle>
                                        <AlertDescription className="text-amber-800 text-xs mt-1 leading-relaxed">
                                            Los datos de la empresa están siendo validados por el departamento académico.
                                        </AlertDescription>
                                    </Alert>
                                )}
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center justify-between border-b pb-4">
                                ...
                                {currentPlacement && currentPlacement.approval_status !== 1 && (
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        className="h-8 gap-2 border-primary/20 hover:bg-primary hover:text-primary-foreground transition-all duration-300"
                                        onClick={form.handleSubmit(handleUpdate)}
                                        disabled={loading}
                                    >
                                        <Save className="size-3.5" />
                                        <span className="text-xs font-bold">{loading ? "Guardando..." : "Sincronizar Cambios"}</span>
                                    </Button>
                                )}
                            </div>
                                        ...
                                        <FormItem className="space-y-2 col-span-1 md:col-span-1">
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {/* Dropdown de verificación — siempre activo */}
                                                    <DropdownMenu>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="space-y-2">
                                            <div className="flex items-center gap-2 px-1">
                                                <FormLabel className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest mb-0 flex items-center gap-1.5">
                                                    {currentPlacement ? 'Área de Desempeño (Cambiar)' : 'Área de Desempeño'}
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                            ...
                                            <div className="flex items-center gap-2">
                                                {/* Input de área siempre readonly — sólo modificable via dropdown */}
                                                <div className="relative flex-1">
                                                    <FormControl>
                                                        <Input
                                                            {...field}
