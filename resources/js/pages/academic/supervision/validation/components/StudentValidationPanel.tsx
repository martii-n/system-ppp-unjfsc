...
export default function StudentValidationPanel({ supervisionId }: StudentValidationPanelProps) {
    ...
    if (isLoading) return <div className="flex-1 flex items-center justify-center text-muted-foreground animate-pulse font-medium">Cargando anexos...</div>;
    if (annexes.length === 0) return <div className="flex-1 flex items-center justify-center text-muted-foreground italic font-medium">No se encontraron anexos para esta supervisión.</div>;
        ...
        <div className="flex-1 flex gap-4 overflow-hidden min-h-0">
            ...
            <div className="flex-1 rounded-xl border bg-card overflow-hidden flex flex-col min-h-[480px]">
                ...
                <div className="flex items-center justify-between h-14 px-5 border-b bg-card shrink-0">
                    <div className="flex items-center gap-3">
                        <div className={`size-8 rounded-md border flex items-center justify-center ${currentAnnex?.latest ? 'bg-primary/5 border-primary/20' : 'bg-muted/50'}`}>
                            <FileText className={`size-4 ${currentAnnex?.latest ? 'text-primary' : 'text-muted-foreground'}`} />
                        </div>
                        <div className="flex flex-col">
                            <span className="text-sm font-semibold leading-none">{currentAnnex?.title}</span>
                            <span className="text-[11px] text-muted-foreground mt-1">
                                {currentAnnex?.latest ? 'Archivo cargado' : 'Sin archivo'}
                            </span>
                        </div>
                    </div>
                    <button
                ...
                <DocumentViewer
                    currentFile={currentAnnex ? { ...currentAnnex, title: currentAnnex.title, latest: currentAnnex.latest } : { code: '', title: '', latest: null }}
                    canUpload={false}
                    onUpload={() => { }}
            ...
            <aside className="w-64 xl:w-72 flex flex-col gap-4 shrink-0">
                ...
                <div className="rounded-xl border bg-card flex flex-col overflow-hidden1">
                    <Tabs defaultValue="validation" className="flex flex-col flex-1">
                        ...
                        <TabsContent value="validation" className="flex-1 p-5 overflow-y-auto mt-0 scrollbar-thin">
                            {currentAnnex && (
                                <ValidationForm
                                    annex={currentAnnex}
                                    onSuccess={() => {
                        ...
                        <TabsContent value="history" className="flex-1 p-5 overflow-y-auto mt-0 scrollbar-thin">
